...
kernel=`uname -r`
echo "Your current Linux kernel version is: $kernel"
...
echo "Select admin task type number:"
OPTIONS="OPERATIONS TARBALLING BACK"
select opt in $OPTIONS; do
    if [ "$opt" = "OPERATIONS" ]; then
        echo "Operations include"
		OPERATIONS="BACKUP REMOVE HIDE+SHRED"
		select opt in $OPERATIONS; do
			if [ $opt = "BACKUP" ]; then
				echo "Enter the full path for the directory you wish to archive:"
				read srcdir
#				echo "The selected directory is:	$backdir"
				echo "Enter the full path for the directory where you wish to store your backup:"
				read targetdir
				cp -r $srcdir $targetdir
				echo 'Going back to menu Options. elect 1 for OPERATIONS, 2 for TARBALLING, 3 for going BACK to main menu'
				break
			elif [ $opt = "REMOVE" ]; then
				echo "Enter the full path for the directory you wish to remove:"
				read remdir
				echo "Do ou really want to remove $remdir forever? (Y/n)"
				read yn
					if [[ $yn = ["Y","y"] ]]; then
						rm -r "$remdir"
					elif [[ $yn = "n" ]]; then
						echo "Back to Operations"							
						break		
						echo "Back to Operations"				
					else
						echo "Wrong input. Back to Operations"	
						break	
					fi
				break
				echo 'Going back to menu Options. elect 1 for OPERATIONS, 2 for TARBALLING, 3 for going BACK to main menu'
			elif [ $opt = "HIDE+SHRED" ]; then
			#	=== ///// emergency file removal script ///// ===
				echo "Enter full path to directory you want to shred."
					read address
					dir="$address*"
					mv "$address" "${address//MDF/.hdir}"  
					new_address="${address//MDF/.hdir}"
					shred -f -u -z "$new_address"
				break
				echo "Going back to menu Options. elect 1 for OPERATIONS, 2 for TARBALLING, 3 for going BACK to main menu"
			else
				echo -e "Wrong input, going back to menu Options. \n Select 1 for OPERATIONS, 2 for TARBALLING, 3 for going BACK to main menu'
."
				break
			fi
		done 
#		echo "U r back to Operations menu. Select 1 for OPERATIONS, 2 for TARBALLING, 3 for going BACK to main menu"
    elif [ "$opt" = "TARBALLING" ]; then
		read "Do you want to create an archive or access it?"
		OPTIONS="TARBALL UNTARBALL BACK"
		select opt in $OPTIONS; do
			if [ $opt = "TARBALL" ]; then
				echo "Enter full path to directory you want to archive."
				read tardir
				echo "Do you want to compress your archive?"			
				OPTIONS="YES NO"
				select opt in $OPTIONS; do
					if [ $opt = "YES" ]; then
			...
			elif [ $opt = "UNTARBALL" ]; then
				echo "Enter full path to directory you want to access."
				read untardir
				echo "Do you need to decompress your archive?"
				OPTIONS="YES NO"
				select opt in $OPTIONS; do
					if [ $opt = "YES" ]; then
						tar -xf "$untardir"
						echo "Done, back to menu TARBALLING"				
						break
					elif [ $opt = "NO" ]; then
						tar -xzf "$untardir"
						echo "Done, back to menu TARBALLING"		
